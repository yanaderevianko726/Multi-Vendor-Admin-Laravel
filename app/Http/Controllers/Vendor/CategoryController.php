...
class CategoryController extends Controller
{
    ...
    function sub_index()
    {
        $categories=Category::where(['position'=>1])->latest()->paginate(config('default_pagination'));
        return view('vendor-views.category.sub-index',compact('categories'));
    }

    function sub_sub_index()
    {
        $categories=Category::where(['position'=>2])->latest()->paginate(config('default_pagination'));
        return view('vendor-views.category.sub-sub-index',compact('categories'));
    }
