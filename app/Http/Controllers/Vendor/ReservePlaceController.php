...
class ReservePlaceController extends Controller
{
    public function index()
    {
        try{
            $reservations = Reservation::whereIn('reserve_status', [0])
            ->whereIn('reserve_type', [1])
            ->where('venue_id',Helpers::get_restaurant_id())
            ->latest()
            ->paginate(config('default_pagination'));
            return view('vendor-views.reserve-places.index',compact('reservations'));
        }
        catch(\Exception $e){
    ...
    public function processing_view(){
        try{
            ...
            ->latest()
            ->paginate(config('default_pagination'));
            return view('vendor-views.reserve-places.processing',compact('reservations'));
        }
        catch(\Exception $e){
            var_dump($e->getMessage());
            die;
    ...
    public function ready_serve(){
        try{
            ...
            ->latest()
            ->paginate(config('default_pagination'));
            return view('vendor-views.reserve-places.ready-view',compact('reservations'));
        }
        catch(\Exception $e){
            var_dump($e->getMessage());
            die;
    ...
    public function completed_view(){
        try{
            ...
            ->latest()
            ->paginate(config('default_pagination'));
            return view('vendor-views.reserve-places.completed-view',compact('reservations'));
        }
        catch(\Exception $e){
            return response()->json(['error' => $e], 201);
        }
    ...
    public function view_reservation(Request $request)
    {
        try{
            ...
            if (isset($order)) {
                ...
                return view('vendor-views.reserve-places.view-reservation', compact('order', 'deliveryMen', 'categories', 'products', 'category', 
