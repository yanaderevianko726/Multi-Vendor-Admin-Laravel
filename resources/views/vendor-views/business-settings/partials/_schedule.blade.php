...
<div class="my-2 p-1 border rounded">
    ...
    @if(isset($data['1']) && count($data['1']))
        @foreach ($data['1'] as $day)
            <span class="btn btn-sm btn-outline-dark m-1 disabled">{{date(config('timeformat'), strtotime($day['start_time']))}} - {{date(config('timeformat'), strtotime($day['end_time']))}} <span class="badge badge-danger rounded-circle cursor-pointer" onclick="delete_schedule('{{route('vendor.business-settings.remove-schedule',['restaurant_schedule'=>$day['id']])}}')">X</span></span>
        @endforeach
    @else
